...
public static class GuiPresets
{
    ...
    private static void DrawProfile(Profile currentProfile, bool drawFallback, bool drawHeader, bool drawGlobalSection)
    {
        ...
        void Buttons()
        {
            ...
            {
                currentProfile.PresetsFolders.Add(new() { Name = $"预设组 {currentProfile.PresetsFolders.Count + 1}" });
            }
            ImGuiEx.Tooltip("添加新预设组");
    ...
    {
        if(ImGui.IsItemHovered() && ImGui.IsItemClicked(ImGuiMouseButton.Right)) ImGui.OpenPopup($"组 {presetFolder?.GUID}");
        if(ImGui.BeginPopup($"组 {presetFolder?.GUID}"))
        {
            if(presetFolder != null)
            {
                ...
                if(presetFolder.HiddenFromSelection)
                {
                    if(ImGui.Selectable("在规则区域显示")) presetFolder.HiddenFromSelection = false;
                }
                else
                {
                    if(ImGui.Selectable("在规则区域隐藏")) presetFolder.HiddenFromSelection = true;
                }
                ...
                {
                    if(ImGui.Selectable("...并将配置文件移动到默认组 (按住CTRL)"))
                    {
                        if(ImGuiEx.Ctrl)
                        {
                            new TickScheduler(() =>
                    ...
                    if(ImGui.Selectable("...并删除包含的配置文件 (按住CTRL+SHIFT)"))
                    {
            ...
            else
            {
                if(ImGui.Selectable("导出到剪贴板"))
                {
                    Copy(EzConfig.DefaultSerializationFactory.Serialize(new PresetFolder() { Name = "导出默认组", Presets = profile.Presets}, false));
                }
            }
        ...
        if(ImGui.BeginTable($"##presets{extraID}", cnt, ImGuiTableFlags.RowBg | ImGuiTableFlags.Borders | ImGuiTableFlags.Resizable | ImGuiTableFlags.Reorderable))
        {
            ...
            for(var i = 0; i < presetList.Count; i++)
            {
                ...
                void FiltersSelection()
                {
                    ...
                    ImGui.Checkbox($"仅选择的##{filterCnt}", ref OnlySelected[filterCnt]);
                ...
                {
                    ImGuiEx.TextV(" ");
                    ImGuiEx.HelpMarker($"此预设的值将作为当前配置文件的默认值使用。");
                }
                else
                {
                    DragDrop.NextRow();
                    ...
                    ImGuiEx.Tooltip("将此预设设为静态，无条件应用于此角色，并忽略所有规则。");
                    ImGui.SameLine();
                    var moveIndex = i;
                    ...
                    {
                        if(ImGui.Selectable("- 主预设组 -", currentProfile.Presets.Any(x => x.GUID == preset.GUID)))
                        {
                ...
                //Glamourer
                {
                    if(C.EnableGlamourer)
                    {
                        ...
                        if(C.StickyGlamourer && C.Sticky)
                        {
                            ...
                            ImGuiEx.Tooltip($"随机化 Glamourer 使用。");
                        }
                        filterCnt++;
                    }
                }
                ...
                //customize+
                {
                    if(C.EnableCustomize)
                    {
                        ...
                        if(isGlobal)
                        {
                            ImGuiEx.HelpMarker("所有已注册的配置都会显示在全局配置中，但只有分配给当前角色的配置才会被实际使用。", EColor.OrangeBright, FontAwesomeIcon.ExclamationTriangle.ToIconString(), false);
                            ImGui.SameLine();
                        }
                        ...
                        if(C.StickyCustomize && C.Sticky)
                        {
                            ...
                            ImGuiEx.Tooltip($"随机化 Customize 使用。");
                        }
                        filterCnt++;
                    }
                }
                ...
                //Honorific
                {
                    if(C.EnableHonorific)
                    {
                        ...
                        {
                            ImGuiEx.HelpMarker("所有已注册的称号都会显示在全局配置中，但只有分配给当前角色的称号才会被实际使用，除非在设置中启用\"允许选择其他角色注册的称号\"。", EColor.OrangeBright, FontAwesomeIcon.ExclamationTriangle.ToIconString(), false);
                            ImGui.SameLine();
                        }
                        ...
                        if(C.StickyHonorific && C.Sticky)
                        {
                            ...
                            ImGuiEx.Tooltip($"随机化 Honorific 使用。");
                        }
                        filterCnt++;
                    }
                ...
                //Penumbra
                {
                    if(C.EnablePenumbra)
                    {
                        ...
                        if(C.StickyPenumbra && C.Sticky)
                        {
                            ...
                            ImGuiEx.Tooltip($"随机化 Penumbra 使用。");
                        }
                        filterCnt++;
                    }
                }
                ...
                //Moodles
                {
                    if(C.EnableMoodles)
                    {
                        ...
                        if(ImGui.BeginCombo("##moodles", preset.Moodles.Select(Utils.GetName).PrintRange(out var fullList, "- 未选择 -"), C.ComboSize))
                        {
                            ...
                            void ToggleMoodle(Vector4 selectedCol, Guid id, string name)
                            {
                                ...
                                if(ImGui.BeginTable($"{id}Moodle", 2, ImGuiTableFlags.NoSavedSettings | ImGuiTableFlags.SizingFixedFit))
                                {
                                    ...
                                    if(cont)
                                    {
                                        ...
                                        ImGui.PopFont();
                                        ImGuiEx.Tooltip($"当预设不再应用时取消此状态效果。");
                                    }
                                    ImGui.EndTable();
                                }
                                ImGui.PopStyleVar();
                            ...
                            if(ImGuiEx.TreeNode(Colors.TabYellow, "Moodle 预设", ImGuiTreeNodeFlags.DefaultOpen))
                            {
                                List<(string[], Action)> items = [];
                        ...
                        if(fullList != null) ImGuiEx.Tooltip("以下所有状态效果/预设将被应用：\n" + fullList);
                        filterCnt++;
                    }
                }
